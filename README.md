...
|---|---|
| 累计训练 | 402 万局自我对弈 |
| 平均分 | ~196,000（从随机水平的 ~900 起步） |
| 8192 达成率 | 88% |
| 16384 达成率 | 40%（实战深搜口径更高） |
...
不想自己训练的话，从 [Releases](https://github.com/Leo2183/2048-td-ai/releases) 下载 `agent-release.npz`（402 万局训练成果），放入项目的 `models/` 目录并改名为 `agent.npz`，然后双击 `启动2048.vbs` 点「AI 演示」即可直接观看高水平对局。

放在 `models/` 下的模型也可以被训练器加载续训——下载后打开 `训练AI.bat` 继续训练，模型会在已有水平上继续变强。
